...
internal class SetKeyTest {
    @MockK
    lateinit var source: Source

    @Test
    fun canParseSetOfInts() {
        every { source.getList(any()) } returns listOf("1", "2", "3")

        val setKey = ListKey("setKey", source, null, emptyList()) { IntegerKey.parse(it) }
            .asSet()

        setKey.resolve() shouldBe setOf(1, 2, 3)
    }
    ...
    fun failsIfCantParse() {
        ...
        shouldThrow<ParseException> {
            ...
            e.shouldHaveMessage("Failed to parse `setKey[1]`: failed to parse '2.0'")
